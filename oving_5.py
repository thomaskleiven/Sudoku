...
class CSP:
    ...
    def isConsistent(self, assignment, color, variable):
        neighbors = self.get_all_neighboring_arcs(variable)
        for neighbor in neighbors:
            if (assignment[neighbor[0]] == color):
                return False    
        return True
    ...
    def backtrack(self, assignment):
        ...
        for value in assignment[variable]:                                   
            ...
            if(self.isConsistent(newAssignment, value, variable)):
                newAssignment[variable] = []
                newAssignment[variable].append(value)
                inference = self.inference(newAssignment, self.get_all_arcs())
                if(inference):
    ...
    def inference(self, assignment, queue):
        ...
        while (len(queue) > 0):
            arcs = queue.pop()
            #print (arcs)
            if(self.revise(assignment, arcs[0], arcs[1])):
 
                if(len(assignment[arcs[0]]) == 0 or len(assignment[arcs[1]]) == 0):
                    #print(assignment)                  
                    return False

                #print (self.get_all_neighboring_arcs(arcs[0]))
                for arc in self.get_all_neighboring_arcs(arcs[0]):
                    #print (arcs[0])
                    #print (arcs[1])
                    #print (arc)                    
                    if(arcs[0] in arc and arcs[1] in arc):
                        continue
                    else:
                        #print("Hi")
                        #print(arc)
